...
```{r}
#The data is tidy because each variable has its own column, each observation has its own row, and each type of observational unit forms a table. The data are in long format they are stored in a single column rather than spread across multiple columns by date.
```
...
```{r}
sydneybeaches_long %>%
  filter(enterococci_cfu_100ml > 200) %>%
