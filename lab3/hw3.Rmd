...
```{r}
name<- c("everest, lhoste, makalu, k2, kangchenjunga")
...
```{r}
mountains<- tibble("names, height_ft, height_meters")
```
...
```{r}
mountains<- tibble ("names, height_ft, height_meters, first_climbed")
```
...
```{r}
summits<- c(12800, 964, 933, 450, 530)
mountains<- tibble ("names, height_ft, height_meters, summits")
```
...
```{r}
fatalities<- c("Everest" = 280,"K2" = 91, "Annapurna I" = 73,"Nanga Parbat" = 68, "Mont Blanc" = 6000)
```
...
"mountains$fatality_rate <- mountains$Fatalities / mountains$Summits"

```

**8. Write your data frame to a .csv file called `mountains_data`.**
```{r}
write.csv("mountains", "mountains_data.csv")
