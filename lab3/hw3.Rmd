...
```{r}
names <- c("everest", "k2", "kangchenjunga", "lhotse", "makalu")
feet <- c(29032, 28251, 28169, 27940, 27838)
meters <- c(8849, 8611, 8586, 8516, 8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <- tibble(names, feet, meters)
```
...
```{r}
summits <- c(12884, 800, 312, 371, 499)
...
```{r}
fatality_rate <- c(344/12884, 96/800, 52/312, 22/371, 50/499)
mountains$fatality_rate <- fatality_rate
```

**8. Write your data frame to a .csv file called `mountains_data`.**
```{r}
write.csv(mountains, file = "mountains_data.csv")
```
