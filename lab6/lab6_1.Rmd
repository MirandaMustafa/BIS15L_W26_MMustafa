...
```{r}
penguins %>% 
  distinct() #344x8 mathces glimplse so no duplicates
```
...
```{r}
mammals<- read_csv("data/mammal_lifehistories_v2.csv") #you can attach %>% clean_names
```
...
```{r}
mammals %>%
  mutate(across(c("order","family")tolower)) #work on specific columns
```
...
```{r}
mammals %>% 
mutate(across(where(is.character), tolower)) #best most clean way
```
...
mammals %>% 
  select(genus, species, newborn) %>% 
  mutate(newmorn_new=ifelse(newborn == -999.00, NA, newborn))
```
## Practice
...
select(species,genus , mass_kg, mass) %>% 
