...
newgabon %>% 
  ...
  labs(title = "Vegetation Among Land Use Types", x = "Land Use Type", y = "Vegetation Richness")
```
...
newgabon %>% 
  ...
  geom_boxplot(mapping = aes(fill = land_use)) +
  labs(title = "Total Species Diversity by Land Use Type", x = "Land Use Type", y = "Total Species Diversity")
```
...
newgabon %>% 
  ...
  geom_point() +
  labs(title = "Vegetation and Mammal Species Richness", x = "Vegetation Richness", y = "Mammal Species Richness")
```
...
```{r}
newgabon %>% 
  select(transect_id, veg_rich, distance) %>% 
  filter(veg_rich > 15.00 | (distance > 50 & distance < 60))
```
...
```{r}
newgabon %>% 
  select(transect_id, veg_rich, rich_mammal_species) %>% 
