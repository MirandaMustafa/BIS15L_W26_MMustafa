...
```{r}
amniota_no_na <-
  read_csv("data/amniota.csv", na="-999") %>% 
  clean_names() %>%
  mutate(adult_body_mass_g=na_if(adult_body_mass_g, 0)) %>%
  mutate(birth_or_hatching_weight_g=na_if(birth_or_hatching_weight_g, 0)) %>% 
  mutate(female_maturity_d=na_if(female_maturity_d,-30258.711 )) %>% 
  mutate(adult_body_mass_g=na_if(adult_body_mass_g, 149000000)) %>%
  mutate(adult_body_mass_g=na_if(adult_body_mass_g, 80000000)) %>%
  mutate(adult_body_mass_g=na_if(adult_body_mass_g, 100000000))
```

```{r}
amniota_no_na <- amniota_no_na %>%
  mutate(adult_body_mass_g = as.integer(adult_body_mass_g))
```
...
# subspecies and female body mass at maturity g have the most NA's
```
...
amniota_no_na %>%
  group_by(class) %>%
  summarise(egg_mass_na = sum(is.na(egg_mass_g))) 
amniota_no_na %>%
  ggplot(aes(x = class)) +
  geom_miss_point(aes(y = egg_mass_g))
# we only have values for reptiles so we can't interpret anything about birds or mammals from this section of data. It might be missing for mammals an absence of egg which makes sense biologically. For birds it might not have been measured. 
```
## Knit and Upload
